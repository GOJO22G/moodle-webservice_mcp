...
class server extends webservice_base_server {
    ...
    public function exception_handler($ex): void {
        ...
        $authfailurecodes = ['invalidtoken', 'accessexception', 'invalidtokenempty'];

        if (in_array($errorcode, $authfailurecodes)) {
            $resourcemetadataurl = $CFG->wwwroot . '/webservice/mcp/protected_resource.php';
